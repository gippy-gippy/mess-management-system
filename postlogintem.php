...
<html>
    ...
    <body>
        ...
        $hashednew=md5($password);
        $con = mysqli_connect("localhost", "root", "", "mess");
        if (isset($username) && isset($password)) {
            setcookie("new_register", 0, time() + 3600);
            $result = mysqli_query($con, "SELECT * FROM admininfo WHERE admin_name='$username' AND passwd='$hashednew' ") or die("couldn't connect to admininfo table from getresponse database");
